...
const { tryMain } = require('./_utils.js')
...
function main() {
  ...
  try {
    ...
    if (!env) throw new Error('validateEnvironment returned falsy')
    console.log('✅ Environment validation passed')
    testResults.passed++
    testResults.details.push({ test: 'Environment Validation', status: 'PASS' })
  } catch (error) {
    console.log('❌ Environment validation failed:', error && error.message ? error.message : String(error))
    testResults.failed++
    testResults.details.push({ test: 'Environment Validation', status: 'FAIL', error: error && error.message ? error.message : String(error) })
  }
  ...
  try {
    ...
    if (typeof sanitized !== 'string') throw new Error('validateInput should return string')
  ...
  try {
    ...
    if (!health || !health.memory || typeof health.memory.percentage !== 'number') {
      throw new Error('Invalid health object')
    }
  ...
  try {
    ...
    console.error('Failed to write results:', e && e.message ? e.message : String(e))
  }
}
